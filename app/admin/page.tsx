...
export default async function AdminPage() {
  ...
  const [{ count: nbCocktails }, { count: nbProducteurs }] = await Promise.all([
    supabase
  ...
  const totalEnAttente = (nbCocktails ?? 0) + (nbProducteurs ?? 0);
    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Link
          ...
          className="group relative rounded-lg border border-border bg-surface p-6 hover:border-accent"
        >
          {(nbCocktails ?? 0) > 0 && (
          ...
          <p className="mt-1 text-sm text-foreground/60">
            {nbCocktails === 0
              ? "Aucune proposition en attente"
              : `${nbCocktails} proposition${(nbCocktails ?? 0) > 1 ? "s" : ""} à valider`}
          </p>
        </Link>
        ...
        >
          ...
          <p className="mt-1 text-sm text-foreground/60">
            {nbProducteurs === 0
              ...
              : `${nbProducteurs} suggestion${(nbProducteurs ?? 0) > 1 ? "s" : ""} à valider`}
          </p>
        </Link>
      </div>
